...
WiegandReaderTrigger = wiegand_reader_ns.class_(
    "WiegandReaderTrigger", automation.Trigger.template(cg.std_string)
)
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Required(CONF_D0): pins.gpio_input_pin_schema,
        cv.Required(CONF_D1): pins.gpio_input_pin_schema,
...
def to_code(config):
    ...
    d0_pin = yield cg.gpio_pin_expression(config[CONF_D0])
    d1_pin = yield cg.gpio_pin_expression(config[CONF_D1])
